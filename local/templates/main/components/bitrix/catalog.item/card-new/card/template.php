...
if($offerArr && $arResult['IS_CART']){
    $IsSkusInBasket = array_intersect($arResult['IS_CART'],$offerArr);
}
?>
...
<div class="product-item">
	...
	<a class="product-item-image-wrapper bg-product-cart" href="<?=$item['DETAIL_PAGE_URL']?>" title="<?=$imgTitle?>"

		data-entity="image-wrapper">
		<? else: ?>
		<span class="product-item-image-wrapper bg-product-cart" data-entity="image-wrapper">
	<? endif; ?>
	...
	<span class="view-button" data-id="<?=$item['ID']?>" >Быстрый просмотр</span>
	</div>
<? else: ?>
	</span>
<? endif; ?>
	
	<div class="bottom-product" data-entity="bottom-block">
	<?
	if (!empty($arParams['PRODUCT_BLOCKS_ORDER']))
	{
		foreach ($arParams['PRODUCT_BLOCKS_ORDER'] as $blockName)
		{
			...
			{
				case 'price': ?>
					<div class="product-item-info-container product-item-price-container line-card-price" data-entity="price-block">
						<div class="price-block">
                            <?
                            $pattern = '/\s+[a-zA-Zа-яА-Я0-9]+$/u';
                            $resultTitle = preg_replace($pattern, '', $productTitle);

                            // Убираем возможные двойные пробелы и пробелы в начале/конце
                            $resultName = trim(preg_replace('/\s+/', ' ', $resultTitle));

                            ?>
							<strong class="product-item-title">
								<? if ($itemHasDetailUrl): ?>
								<a href="<?=$item['DETAIL_PAGE_URL']?>" title="<?=$resultName?>">
									<? endif; ?>
									<?=$resultName;?>
									<? if ($itemHasDetailUrl): ?>
								</a>
				...
				case 'sku':
					if ($arParams['PRODUCT_DISPLAY_MODE'] === 'Y' && $haveOffers && !empty($item['OFFERS_PROP']))
					{
						?>
						<?
                        $material = $item['OFFERS'][0]['PROPERTIES']['Material']['VALUE'];
                        $madeIn = $item['OFFERS'][0]['PROPERTIES']['Proizvoditel']['VALUE'];
							/*Откуда брать данные пока непонятно*/
						?>
                        <? if($material && $madeIn):?>
						<div title="<?=$material?>, <?=$madeIn?>" class="product-info"><?=$material?>, <?=$madeIn?></div>
													...
													foreach ($skuProperty['VALUES'] as $value)
													{
														...
														else
														{
															...
															<li class="product-item-scu-item-text-container" title="<?=$value['NAME']?>"
																...
																<span class="product-item_scu-block">
																	<span class=""><?=$value['NAME']?></span> см
																</span>
															</li>
															<?
														}
