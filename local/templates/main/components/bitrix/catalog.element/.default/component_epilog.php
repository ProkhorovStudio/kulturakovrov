...
$offerNum = false;
$targetOffer = (int)$request->get('offer');
if ($targetOffer  && !empty($templateData['OFFER_IDS']) && is_array($templateData['OFFER_IDS'])) {
    $offerNum = array_search($targetOffer, $templateData['OFFER_IDS']);
}


if ($offerNum !== false && !empty($templateData['JS_OBJ'])) { ?>
    <script>
        BX.ready(function(){
            if (window.<?=$templateData['JS_OBJ']?>) {
                window.<?=$templateData['JS_OBJ']?>.setOffer(<?=$offerNum?>);

                const url = window.location.href.replace(/(.*\/.*\/.*\/.*)\?offer=(.*)/, '$1$2/');
